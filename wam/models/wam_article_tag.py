...
class WamArticleTagCategory(models.Model):
    _name = "wam.article.tag.category"
    _description = "Article Tag Category"
    _order = "sequence"

    name = fields.Char("Name", required=True, translate=True)
    sequence = fields.Integer('Sequence', default=0)
    tag_ids = fields.One2many('wam.article.tag', 'category_id', string="Tags")
...
class WamArticleTag(models.Model):
    ...
    name = fields.Char("Name", required=True, translate=True)
    sequence = fields.Integer('Sequence', default=0)
    category_id = fields.Many2one("wam.article.tag.category", string="Category", required=True, ondelete='cascade')
